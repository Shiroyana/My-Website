...
> licensed in your state. Before you send this to a paying client, get it
> reviewed. A one-time contract review from a small-business attorney
> typically runs $150–400 and is worth it the first time real money is on
> the line. Fill in every `[bracket]` before use.
...
- [ ] **Growth Partner** — one-time build ($`[amount]`) + $`[amount]`/month ongoing
- [ ] **$0 Down** — $`[amount]`/month, 6-month minimum term (see §5)
...
The scope above reflects what's included. Additional pages, redesigns
after design approval, or new features requested after work begins will be
quoted separately and require Client's written approval before work
starts. Minor content/copy tweaks within the "ongoing" tier are included;
structural changes are not.

## 5. Payment

**One-time packages:** 50% deposit due at signing to begin work; remaining
50% due before the site goes live. Monthly fee begins on launch date.

**$0 Down packages:** No deposit. Monthly billing begins on launch date.
**Minimum term: 6 months** from launch date, billed monthly. After the
6-month minimum, the agreement continues month-to-month until cancelled
by either party with 30 days' written notice.

**All packages:** Monthly fees are billed on the 1st of each month via
`[Stripe/Square/etc.]`. Payments more than 10 days late may result in the
site being taken offline until the account is current. A $`[amount]`
late fee applies after 15 days.

**Cancellation (month-to-month, after minimum term):** Either party may
cancel with 30 days' written notice. No refunds for the current billing
period. Client may request a full export of their site files upon
cancellation, provided the account is current.

## 6. Ownership & Usage

Upon full payment, Client owns the final website design, content, and
...
information shared during the engagement (pricing, customer data,
internal processes) and not disclose it to third parties, except as
required to perform the work (e.g., hosting providers).
...
Either party may terminate for material breach not cured within 14 days
of written notice. Client remains responsible for payment for work
completed through the termination date.

## 12. Governing Law

This Agreement is governed by the laws of the State of `[your state]`.
Disputes will be resolved in the courts of `[your county]`.
